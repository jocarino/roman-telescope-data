...
**US / NGSS.** The strong alignments are:

- **HS-PS4-1** (frequency/wavelength/speed relationships) and **HS-PS4-3** (wave vs particle models of EM radiation) — the spectrum plot is a direct read.
- **HS-ESS1-2** — "construct an explanation … based on astronomical evidence of **light spectra**". Spectra-as-evidence is explicitly named.
...
- Middle school: **MS-PS4-2** (wave interaction with matter — absorption) covers the methane story at a lower level.
...
**Portugal / EU (maintainer's home advantage).** Portuguese *Física e Química A* covers stellar spectra, black-body radiation and atomic emission in the 10.º-ano chemistry strand ("Das Estrelas ao Átomo"), with waves and electromagnetism in 11.º ano. **Flag: I could not verify the exact *Aprendizagens Essenciais* wording from search — check the DGE PDF at `dge.mec.pt` before printing any alignment claim.** The real opening is institutional rather than curricular: **ESERO Portugal** is run by **Ciência Viva** out of the **Pavilhão do Conhecimento in Lisbon** (esero.pt), it is ESA's national teacher-facing office, it publishes a "Recursos de Professores" library, and it runs teacher training. A Lisbon-based maintainer can walk in. That is an advantage almost nobody else pitching a hobby site has, and it is worth more than any of the online listing sites below.
...
3. **Solar-system anchor activity — "does the model agree with the photograph?"** *~1 weekend.* The signature piece and the one worth doing properly. Students compare the site's computed colour for Jupiter/Saturn/Neptune/Uranus/Mars against the real photograph, find where it agrees and where it does not, and write down *why a disagreement is information rather than failure*. This is SEP-2 and SEP-7 in one page, and it is the activity astroEDU is most likely to accept. 50 minutes.
...
6. **A 10-minute guided tour framed as a lesson starter.** *~1 evening.* We already ship guided tours; this is a copy job plus one teacher note. "The ten darkest worlds" is already a good hook. Lowest effort of all, but it only pays off once someone is already on the site.
...
|---|---|---|---|
| **astroEDU** (IAU Office of Astronomy for Education) | Email `astroedu@astro4edu.org` to propose first; if accepted you get a template to fill in and return. Two referees — one research scientist, one educator. Published **CC BY 4.0**. | Yes | **Do this first.** Peer review gives credibility nothing else on this list gives, and the CC BY requirement matches our licence anyway. Slow (months) — submit activity 3 and forget about it. |
...
| **ASP — *Universe in the Classroom*** | Quarterly free newsletter for grades 3–12, co-sponsored by AAS/CASCA/IPS, claims 20,000+ teachers. Pitch the editor via astrosociety.org. | Yes | **Strong.** A single mention lands in exactly the right inbox. Newsletter cadence means a long wait; pitch once, don't chase. |
...
**Astronomy on Tap** runs in dozens of cities worldwide (10+ years old as of 2026), always needs speakers, is aimed at a pub audience, and several chapters stream or run online — LA moved virtual in 2020, San Antonio streams to Facebook/YouTube. **Flag: there is no formal speaker-application process.** The central address is `astronomyontap@gmail.com` and the main site has a "Request a Satellite" page, but in practice you email individual chapter organisers, who are usually named on that chapter's page or Eventbrite listing. Email five chapters, expect one or two yeses. Remote is the ask — say so up front, since it removes their travel-budget objection entirely.
...
> Hi [name],
>
> I would love to give a 10-minute talk at Astronomy on Tap [city]. I am happy to do it remotely, so there is no travel to arrange.
>
...
- **Late August.** Submit to TES and OER Commons, email ESERO Portugal, ASP, the OAE Portuguese coordinator. One wave.
...
- **Curriculum-alignment overclaim.** Reflected-light colour is not on the AQA spec, and the Portuguese *Aprendizagens Essenciais* wording is unverified. Claiming alignment we do not have destroys credibility with exactly the audience that checks. State alignment conservatively; say "enrichment" where it is enrichment.
