...
## Blocking prerequisites — do not submit until these are true

These are not polish; each one is a thing the thread will find.

- **A0. The CGI band configuration on the site is wrong.** `pipeline/config.py` still ships the pre-flight four-band model (575/10%, 660/6%, 730/6%, 835/15%). Per the January 2025 CPP flight Primer, Band 2 at 660 nm is **not in the flight configuration at all**, Band 3 is 730 nm at **15%**, Band 4 is **825 nm at 10%**, and *"only observations in Band 1 with the hybrid Lyot coronagraph are formally supported; all other modes … will be supported on a 'best effort' basis."* The signature feature is currently built on a superseded config. Fix it before submitting — see [15-roman-launch.md](./15-roman-launch.md). Posting a Show HN whose headline feature states a wrong instrument config, on the one site whose entire pitch is honesty, is the worst available outcome.
- **A1. The credits/licence obligation ([13](./13-credit-the-scientists.md)) is discharged.** The hub marks it *blocking and overdue*; there is an unmet CC BY attribution obligation. HN is the single most likely audience to notice an attribution failure, and it converts the thread from "interesting pipeline" to "he took someone's grids without credit". There is currently no `credits` template in `web/templates/`.
- **A2. `/press` exists ([02](./02-press-kit.md)) and planet-page SEO has shipped ([03](./03-seo-planet-pages.md)),** per the hub's sequencing. Journalists read HN the same day; there is currently no `press.html`.
- **A3. Something exists to catch the traffic.** The Risks section assumes a visible repo link, dataset link and an RSS/Atom feed. There is no feed template today. Decide what the catch is, or accept that the spike leaves nothing behind.

## What to do

**Pre-launch (one evening, the week before)**

1. **Confirm the account.** There is no published karma threshold for submitting, and Show HN itself has no karma requirement — but submissions from brand-new and very-low-karma accounts are frequently caught by HN's anti-abuse filter and land dead, requiring other users (31+ karma) to *vouch* them back to life, which for an unseen new post effectively never happens. Use an established account with real comment history. If there isn't one, spend two weeks commenting substantively on unrelated threads first. Do not create a new account for this.
...
**Hours 6–24: the part where you are asleep**

A thread submitted at 13:00 UTC is still going at 03:00 Lisbon time, and the US afternoon wave — often the biggest — lands squarely in the middle of a Portuguese night. Six hours of presence covers the launch, not the thread. Plan for the rest:

- **Post a timezone comment before you go to bed, not after.** One line, near the top of the thread: *"Author — I'm in Portugal, it's past midnight here, so I'm out for ~7 hours. I'll answer everything in the morning."* HN reads this as normal and it converts "author abandoned the thread" into "author is a person". Never let the silence be unexplained.
- **Triage on waking, oldest-first.** Answer the substantive unanswered questions in the order they were asked. Late good answers still get read — HN threads keep receiving traffic for days.
- **Pre-decide the one thing you'd get out of bed for:** a factual error on the site being pointed out, or an outage. Nothing else is worth the sleep, and a tired reply is how the flamewar detector gets fed.
- **If it happens on a workday**, the failure mode is worse than being asleep, because you'll answer in snatched five-minute windows and it will show. Either take the day, or don't submit that day.
...
17. **Do not edit the title once votes are coming in** — the author edit window is roughly **two hours** and closes on its own, while moderators can retitle at any time. Accept their rewrite silently.
...
HN's title field is **80 characters** (verified — the limit is long-standing and enforced by the submit form; "Show HN: " counts toward it). Count before you submit. Character counts below were re-counted programmatically; the en dash in #1 is one character.
...
4. **`Show HN: I computed a colour for all 5,700 known exoplanets`** (59)
...
5. **`Show HN: What colour is every known exoplanet? (Modelled, not photographed)`** (75)
...
>
> The feature I actually built this for: the Roman Space Telescope's coronagraph sees a handful of narrow visible bands, and only one of them — Band 1, 575 nm, 10% wide, hybrid Lyot — is a formally supported mode. Everything else is best effort. Toggle "as Roman would see it" and you get the colour reconstructed from just those samples, next to the full-spectrum colour. That difference is the question the whole project is asking.
>
...
Careful — as written this concedes something that probably isn't true, and a gamut pedant will notice. A broadband reflectance spectrum multiplied by a broadband stellar illuminant is inherently low-purity; those colours land well *inside* sRGB, not outside it. Almost nothing here is genuinely out of gamut. What actually produces negative channel values is the luminance normalisation (answer 5) pushing a low-Y colour up until a channel overshoots, plus the XYZ→sRGB matrix on near-neutral dark colours. So the honest answer is: *"Very little of this is truly out of gamut — the chromaticities are low-purity by construction. What I do clip is the overshoot introduced by normalising luminance, and I clip per channel, which shifts hue slightly rather than desaturating toward the white point."* Then state which you chose (per-channel clip vs chroma-preserving desaturation) and note the underlying XYZ and the 5 nm spectrum are in the dataset, so a Display P3 or Rec. 2020 render is yours to redo.
...
It does. Don't quantify this as "a few percent" unless you have actually measured it — a physicist will ask "a few percent of *what*, and in what metric", and for the cool end of the catalogue the honest answer is much worse than a few percent. For FGK hosts a Teff blackbody is a decent visible-band illuminant. For M dwarfs it is not an approximation worth defending: TiO and VO bands carve up the visible SED and a blackbody badly overpredicts the blue, and M dwarfs are a large share of the catalogue. The defensible answer is: *"Blackbody-from-Teff is the v1 illuminant. It's fine for solar-type hosts and genuinely poor for M dwarfs, which is a big fraction of these planets. PHOENIX/Kurucz is the upgrade and it's the next thing I'm changing."* If you want a number, compute ΔE00 against a PHOENIX spectrum for one Sun-like and one M-dwarf case before launch day and quote those two — one measured number beats a hand-waved range.
...
It can't reconstruct the spectrum, and it isn't trying to — it's asking how much *colour identity* survives a handful of samples, which is a different and answerable question. I integrate the full reflected spectrum through the CGI bandpasses as top-hats and run the same CIE step. Per the January 2025 Coronagraph Community Participation Program flight Primer the configuration is Band 1 at 575 nm/10% (narrow- and wide-FoV imaging), Band 3 at 730 nm/15% (slit + R~50 prism), and Band 4 at 825 nm/10% (wide-FoV imaging) — and **only Band 1 with the hybrid Lyot coronagraph is formally supported; the rest are best effort.** So the honest version of the question is not "what do four bands buy you", it's "what does *one* 10%-wide photometric point buy you, and what does each best-effort band add on top". Sometimes the answer is "almost all of the colour identity" and sometimes it's "the methane signature vanishes entirely" — that contrast is the point of the feature.

> Note: the 660 nm band that appears in older community material is **not** in the flight configuration. If you see it anywhere on the site or in this doc, that is the stale config ([15-roman-launch.md](./15-roman-launch.md), prerequisite A0) and it must be fixed before submitting.
...
**13. "What's your white point? Are you chromatically adapting to the host star or to D65?"**
This is the first question a colour scientist asks, and there is no hedging past it — fill in the actual convention from `pipeline/cie.py` before submitting. The two choices give visibly different galleries: adapt to the host star and a neutrally-reflecting planet around an M dwarf comes out grey (you are showing *the planet's* reflectance); keep D65 and the same planet comes out deep orange (you are showing *what a human eye adapted to daylight would see if teleported there*). Neither is wrong, both are defensible, and stating which one you chose and why is the answer. The related follow-up — "so is your Roman-vs-full-spectrum comparison adapted the same way in both legs?" — must also be yes, or the comparison isn't one.

**14. "Geometric albedo is defined at zero phase. You have a phase slider. Which is it?"**
Fair catch on terminology. Geometric albedo *is* the zero-phase quantity; at non-zero phase what's being computed is the planet/star flux ratio — albedo times the phase function — not a geometric albedo. If the site's copy says "geometric albedo" next to a phase control, that copy is wrong and should be fixed before submitting rather than defended in the thread.

**15. "How many of these 5,700 planets actually have measured parameters? Most exoplanets don't have a measured radius, let alone a mass."**
Have the exact counts ready as numbers before you submit; do not answer this one from feel. `pipeline/catalog.py` already tags equilibrium temperature provenance as `measured` / `computed` / `assumed` — count each bucket, and count how many records have measured radius *and* mass *and* Teq versus how many lean on a mass–radius relation. The strong answer is "N of 5,700 are measured on all three, M are partly inferred, K are almost pure inference and are marked as such" — a specific number is credible; "most of them, roughly" gets torn apart, because the person asking has the archive open in another tab.
...
- **Sunday, 12:00 UTC** = **13:00 Lisbon (WEST)**. Verified: Myriade's analysis of 157k+ Show HN posts since 2009 (HN BigQuery dataset, "breakout" = reaching 30 points, ~top 10%) gives **Sunday 11.75%** against weekdays at **9.45–9.90%**, a peak hour of **12:00 UTC at 12.2%**, and a golden window of **11:00–16:00 UTC** (all above 10.5%); worst hours are 03:00–07:00 UTC. **Note the tension: the only evidence cited in this doc ranks Sunday first, yet the doc calls Tuesday/Wednesday "primary".** The defence for the weekday slot is a higher absolute ceiling, not a higher hit rate, and that defence is asserted rather than measured. Two honest caveats on the Myriade numbers: they average across 17 years of a site that has changed a lot, and breakout rate is a rate over posts, so weekend and weekday populations may simply differ in kind. Treat the effect as real but modest, and let presence decide.
...
- **Resubmission.** HN's FAQ, verbatim: *"If a story has not had significant attention in the last year or so, a small number of reposts is ok."* Note the window is stated as **a year or so**, not weeks. And the mechanic that makes "wait" non-optional: **resubmitting a URL that HN still has on file counts as an upvote for the existing submission rather than creating a new one** — so a quick re-fire silently does nothing but bump a dead post. If it dies with under ~10 points and a couple of comments, wait — weeks at minimum — and resubmit once, ideally with a different title from the ranked list. If it got real traction (50+ points), do **not** resubmit; the next submission must be a genuinely new artefact (the open dataset, the Roman launch tie-in). You may also email hn@ycombinator.com about the second-chance pool, but dang's stated preference is that pool suggestions come from readers rather than authors — so ask, briefly, once, and accept a no.
