...
def cmd_mprocs(args) -> int:
    ...
    assigned = assign_ports()
    lines = [
        "# generated by `python tools/exohub.py mprocs` — one pane per worktree",
        "procs:",
        '  "▟ dash":',
        f'    shell: "while true; do clear; python {script} dash; sleep 3; done"',
    ]
    for t in trees:
        port = assigned[t.branch]
        lines += [
            f'  "{t.label} :{port}":',
            f'    shell: "python {script} serve"',
            f'    cwd: "{t.path}"',
        ]
